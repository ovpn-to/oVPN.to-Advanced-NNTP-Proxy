...
def IDLE_TIMER():
    ...
    if IDLE_TIMER_RUNNING is True:
        ...
        for cid, val in CONN_ID.items():
            kill = False
            if val["AUTH"] is False and int(time.time() - val["TIME"]) > MAX_TEMP_IDLE_TIME:
                kill = True
                dbg(1, "def IDLE_TIMER: killed non-auth idle conn_id = '%s'" % (cid))
            elif val["AUTH"] is True and int(time.time() - val["LAST"]) > MAX_CONN_IDLE_TIME:
                kill = True
...
class Frontend(LineReceiver):
    ...
    def makeConnection(self, transport):
        ...
        if CURRENT_TEMP_CONNS > MAX_TEMP_CONNS:
            ...
            return self.transport.loseConnection()
        elif CONFIG["BACKEND"]["OFFLINE"] is True:
    ...
    def connectionLost(self, reason):
        ...
        if self.auth_user in FRONTEND_USER_CONNS:
        ...
        if self.remove_conn_on_lost is True:
            CURRENT_TEMP_CONNS = max(0, CURRENT_TEMP_CONNS - 1)

        if CONN_ID[self.conn_id]["AUTH"] is True and CONN_ID[self.conn_id]["USER"].startswith("READER-"):
    ...
    def lineReceived(self, line):
        ...
        if line.upper().startswith('AUTHINFO USER '):
            ...
            if len(data) is 3:
                ...
                self.auth_user = ''
            if self.auth_user in LOCAL_USERS:
            ...
            if len(data) == 3 and LOCAL_USERS.get(self.auth_user) == hashlib.sha256(data[2].strip()).hexdigest():
                if self.auth_user not in FRONTEND_USER_CONNS:
                    FRONTEND_USER_CONNS[self.auth_user] = 1
                else:
                    FRONTEND_USER_CONNS[self.auth_user] += 1
                CURRENT_BACKEND_CONNS += 1
                CURRENT_TEMP_CONNS = max(0, CURRENT_TEMP_CONNS - 1)
                if self.auth_user in USER_CONNECTIONS:
    ...
    def _LineToBackend(self, line):
        ...
        if CONN_ID[self.conn_id]["BACK"] is False:
            # silent return if conn_id has no backend conn yet
            return
        elif lineisascii(line) is False:
        ...
        if F_CMD is not False and lined is False:
    ...
    def update_traffic_db(self):
        ...
        if UPDATE_TRAFFICDB_RUNNING is True:
            return
...
class Backend(LineReceiver):
    ...
    def lineReceived(self, line):
        ...
        if CONN_ID[conn_id]["BACK"] is True:
            response = 0
            if lineisascii(line) is True:
        ...
        if CONN_ID[conn_id]["BACK"] is False:
...
if CONFIG["BACKEND"]["logs"] is True:
